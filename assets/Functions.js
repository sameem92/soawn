...
AddFunc(
 ...
 function AddNew  (collection, data, SuccessFunc, ErrorFunc) {
    ...
    app.collection(collection).add(data).then( (d) => SuccessFunc(d)).catch((e) => ErrorFunc(e));
}
...
export function IsNormalPersonEmailExist(collection, check_DATA, thenFunc) {
    ...
    app.collection(collection).where(check_DATA[0], check_DATA[1], check_DATA[2]).onSnapshot((data) => {
...
export function Log(collection, check_DATA_USER, check_DATA_PASSWORD, thenFunc) {
    var app = FirebaseApp.firestore();

    app.collection(collection).where(check_DATA_USER[0], check_DATA_USER[1], check_DATA_USER[2]).where(check_DATA_PASSWORD[0],check_DATA_PASSWORD[1],check_DATA_PASSWORD[2]).onSnapshot((data) => {
...
export function Firebase_Update (collection, filter, DATA, SuccesFun = () => {}, ErrorFun = () => {}) {
    window.XMLHttpRequest = window.tempXMLHttpRequest;

    FirebaseApp.firestore().collection(collection).where(filter[0], filter[1], filter[2]).get().then((d) => {
      id = d.docs[0].id;
      FirebaseApp.firestore().collection(collection).doc(id).update(DATA).then((d) => SuccesFun(d)).catch((e) => ErrorFun(e));
...
export function uploadImage (uri, type, fileName, SuccessFun = () => {}, ErrorFunc = () => {})  {
